...
## ADR-018: Sentry hand-authored following current `@sentry/nextjs` conventions — wizard could not run non-interactively
...
## ADR-019: `beforeSend` drops ZodError and `AppError("invalid_input")` events instead of reporting them
...
## ADR-020: Sentry test routes gated on "not a real Vercel Production deployment," not on Node's `NODE_ENV`
